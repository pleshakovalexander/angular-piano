...
export class OctaveModelHelper {
  ...
  getModel(octaveName: string): OctaveViewModel[] {
    ...
    octaveNames.forEach(octave => {
      const currentMidiNotes = notesInOctave.map(n => toMidi(n + octave.index));
      const m: OctaveViewModel = {
        name: octave.name,
        notes: this.allNotes.map(name => {
          const noteName = name + octave.index;
          const noteMidi = toMidi(noteName) || 0;
          const noteDegree = this.noteDegreeToRoman(currentMidiNotes.indexOf(noteMidi) + 1);
...
export class Note {
  ...
  constructor(name: string, degree: string = '', midi: number = 0, ) {
    ...
    this.degree = degree;
    this.midi = midi;
    this.cssClassObj = this.getCSSClassToDefault();
  }

  public getCSSClassToDefault(): any {
